...
授权操作员可以发送 `/panel`，进入“本地资源”，打开对应资源并选择“处理不确定投递”。
面板提供两种显式恢复操作：

- 只有在目标 Telegram 会话中确认媒体确实存在后，才选择“确认已送达”。这会写入本地
  投递记录、结束任务，并允许后续保留策略继续判断。
- 若确认目标中没有该媒体，可以选择“强制重新发送”。任务会立即重新入队，但必须接受
  Telegram 中可能出现重复消息的风险。

两种操作都使用状态版本校验，并在 SQLite 中记录操作员、原错误、目标和处理时间；任何
状态变化都会使旧确认按钮失效。系统仍不会自动处理 `uncertain`。
