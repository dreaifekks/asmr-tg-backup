...
`sources.toml` 与 SQLite 数据库。此时还没有媒体来源：启动服务后，向 bot 发送
`/panel`，再添加一个 YouTube 或 Twitch 来源即可。Panel 会把来源和过滤器写入可编辑的
`sources.toml`，SQLite 只保存同步镜像和运行状态。添加 Twitch 前还需要按照
