...
`[proxy]` controls the network path for provider discovery/API requests and
`yt-dlp` probe/download commands. The shipped baseline is explicitly disabled:
...
The application-scoped proxy is deliberately not injected into Telegram
delivery or control requests, so a local Bot API such as
`http://127.0.0.1:18081` is unaffected by `[proxy]`. Process-wide proxy
variables, if separately configured by the operator, retain their normal
`urllib`/curl behavior.

The recommended setup is an HTTP proxy for both media scopes:
...
An HTTP proxy can carry both ordinary HTTP and HTTPS requests, so this covers
YouTube/RSS discovery, Twitch API requests, and yt-dlp downloads without using
process-wide `HTTP_PROXY` variables that could also affect unrelated traffic.
Leave `url` and `url_env` empty to preserve the previous ambient-environment
behavior without adding an application-scoped proxy.
The shipped baseline keeps `sources` and `downloads` set to `false`; explicitly
enable only the intended scopes when configuring a non-empty proxy URL.

Set only one of `url` or `url_env`. If the proxy URL contains credentials,
reference an environment variable instead of putting the secret in TOML:
...
```dotenv
ASMR_TG_BACKUP_PROXY=http://user:password@127.0.0.1:7890
```

Keep that assignment in the mode-`0600`
`~/.config/asmr-tg-backup/env` loaded by the shipped user service. A configured
`url_env` whose variable is missing is a startup error. Verify that its value is
non-empty before starting the service; an empty value is also a startup error.

SOCKS is available only for yt-dlp downloads:
...
Source discovery and provider API calls do not support SOCKS. Twitch live HLS
can delegate fragment transfer to ffmpeg, whose use of the configured SOCKS
proxy is not guaranteed; use the HTTP configuration above when Twitch live
recording must consistently follow the proxy. Restart the service after
changing this static configuration.
